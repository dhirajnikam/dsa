...
if __name__ == "__main__":
    ...
    assert find_order(2, [[1, 0]]) == [0, 1], 'Check: find_order(2, [[1, 0]]) == [0, 1]'
    p = [[1, 0], [2, 0], [3, 1], [3, 2]]
    assert valid(find_order(4, p), 4, p), 'Check: valid(find_order(4, p), 4, p)'
    assert find_order(1, []) == [0], 'Check: find_order(1, []) == [0]'
    assert valid(find_order(3, []), 3, []), 'Check: valid(find_order(3, []), 3, [])'
    assert find_order(2, [[1, 0], [0, 1]]) == [], 'Check: find_order(2, [[1, 0], [0, 1]]) == []'
    assert find_order(3, [[0, 1], [1, 2], [2, 0]]) == [], 'Check: find_order(3, [[0, 1], [1, 2], [2, 0]]) == []'
    p = [[1, 0], [2, 1], [3, 2], [4, 3], [5, 4]]
    assert find_order(6, p) == [0, 1, 2, 3, 4, 5], 'Check: find_order(6, p) == [0, 1, 2, 3, 4, 5]'
    p = [[4, 0], [4, 1], [5, 2], [5, 3], [6, 4], [6, 5]]
    assert valid(find_order(7, p), 7, p), 'Check: valid(find_order(7, p), 7, p)'
    print("ok")
